...
wget --no-check-certificate -O /tmp/lazy.txt https://gitee.com/kysdm/xwhyc-rules/raw/master/lazy.txt
 if [ "$?" == "0" ]; then
...
#wget --no-check-certificate -O /tmp/video.txt https://gitee.com/kysdm/xwhyc-rules/raw/master/video.txt
# if [ "$?" == "0" ]; then
#    echo_date 下载video文件成功，判断规则是否有更新中...
#   else
#    echo_date 下载video文件失败，连接失败，尝试挂代理后重试
#    rm -rf /tmp/video.txt
# fi
