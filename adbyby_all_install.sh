...
auto_adupdate1_install(){
    ...
    #    echo "	option custom_cron_table '0 */3 * * * /usr/share/adbyby/adupdate.sh >> /tmp/log/adupdate.log 2>&1'" >> $cron
...
# 	sed -i '/adbyby/d' $crontab
#     echo "" >> $crontab
#     echo -e "\n0 */3 * * * /usr/share/adbyby/adupdate.sh >> /tmp/log/adupdate.log 2>&1" >> $crontab
#     /etc/init.d/cron restart 
#     echo -e "${Info} 写入完成"
# }
auto_adupdate3_install(){
